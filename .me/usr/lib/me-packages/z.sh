if [[ $1 == install ]]; then
  ...
  if (( $? == $ME_ES_UNKNOWN_RELEASE )); then
    ...
    git clone --branch master --single-branch --depth 1 \
      https://github.com/rupa/z.git "$ME_SRC_DIR/z"
    ln -sf "$ME_SRC_DIR/z/z.1" "$ME_MAN_DIR/man1/z.1" && mandb &> /dev/null
  fi
  ...
  elif [[ -r $ME_SRC_DIR/z/z.sh ]]; then
    source "$ME_SRC_DIR/z/z.sh"
  fi
